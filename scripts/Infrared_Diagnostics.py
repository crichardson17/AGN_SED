...
for root, dirs, files in os.walk(rootdirectory, topdown=False):
    for name in files:
        if name.startswith('Hden25') and name.endswith('.lin'):
            ...
            dfs.append(pd.read_csv(os.path.join(root, name), delimiter="\t", usecols=['NE 5 14.32m', 'NE 2 12.81m','NE 3 15.55m','O  4 25.88m','NE 5 24.31m']))
            d = pd.concat(dfs, ignore_index=True)
            
d['Temperature']=d2

d['O IV / Ne II'] = d['O  4 25.88m'] / d['NE 2 12.81m']
d['Ne V / Ne II'] = d['NE 5 14.32m']/ d['NE 2 12.81m']
